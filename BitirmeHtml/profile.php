...
while($row=mysqli_fetch_array($var1))
{
 ...
 echo '<img src="'.$image_name.'" width="100" height="100">';
  ...
  echo '<img src="noldu.jpeg" width="100" height="100">';
}
  ...
  <div class="form">
    ...
    $sorgu= mysqli_query($conn,"SELECT user_movie FROM movie WHERE user_mail='$user_mail' and status=1 ");
       ...
       while($row1 = $sorgu->fetch_assoc()) : ?>
            ...
            <form action="moviepage.php" method="get" >          
            <button style="width: 200px;" type="submit" name = "movietitle" class="rem" value = <?php echo '"'.$row1['user_movie'].'"' ?>   /> <?php echo $row1['user_movie'] ?> </button>
            </form>
              <form action="removemovie.php" method="post">          
            <button style="width: 30px;" type="submit" name = "movietitle"  value = <?php echo '"'.$row1['user_movie'].'"' ?> /> X </button>
            </form>
            <?php endwhile;
